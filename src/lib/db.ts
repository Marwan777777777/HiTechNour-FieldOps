...
/**
 * Run `fn` on a single connection inside BEGIN/COMMIT.
 * Required for SELECT … FOR UPDATE — a pooled query per statement would
 * otherwise lock and unlock on different connections.
 */
export async function withTransaction<T>(fn: (sql: Sql) => Promise<T>): Promise<T> {
  if (typeof window !== "undefined") {
    throw new Error("@/lib/db withTransaction is server-only");
  }
  if (dbSource === "pglite") {
